...
class TestSectionConstruction:
    ...
    def test_get_section_rolls(self, driver):
        ...
        WebDriverWait(driver, 4).until(EC.visibility_of_element_located(SectionConstructionLocators.TITLE_CONSTRUCTOR))
    ...
    # Тест перехода из Личного кабинета в раздел "Конструктор" по клику на логотип
    def test_click_logo_to_constructor_from_personal_account_page(self, driver, login):
        ...
        title_constructor = WebDriverWait(driver, 4).until(EC.visibility_of_element_located(SectionConstructionLocators.TITLE_CONSTRUCTOR)).text
        assert title_constructor == 'Соберите бургер'

    # Тест перехода из Личного кабинета в раздел "Конструктор" по клику на кнопку "Конструктор"
